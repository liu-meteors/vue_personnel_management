...
<script>
    export default {
        ...
        methods: {
            ...
            getDepLeaveNow(dep){
                const me=this
                axios.get('http://localhost:8181/getLeaveByDepMonth/'+dep).then(function (resp) {
                    console.log(resp.data)
                    me.tableData=resp.data
                })
            },
            ...
            handleClick(tab, event) {
                ...
                if (tab.name=='first'){
                    this.getDepLeaveNow(sessionStorage.getItem('dep'))
                }else {
                    this.getAllDepLeave(sessionStorage.getItem('dep'))
                }
            },
            setTag(tag){
                if (tag=='已批准'){
